...
label_names = train_data.columns
...
parser.add_argument("--path", default="bert-base-uncased", type=str, required=True,
...
y_true = []
with torch.no_grad():
    for data_point in test_data.to_numpy():
...
print(classification_report(np.array(y_true), np.array(y_predicted)))
